  ...
  <q-scroll-area :visible="false" class="window-height">
    ...
    <q-dialog v-model="slt">
      <q-card class="column items-center no-wrap" style="width: 475px; border-radius: 10px;">
        ...
        <div v-if="solicitud.hitos.length" class="full-width q-px-md q-mb-md">
          <div class="q-mb-sm">
...
export default {
  ...
  methods: {
    ...
    getSltUser () {
      this.sltProgress = []
      for (var i = 0; i < 4; i++) {
        this.$api.put('solicitudes_cliente/' + this.user._id, i === 0 ? { status: i } : i === 1 ? { status: i } : i === 2 ? { status: i } : i === 3 ? { status: i } : {}).then(res => {
          if (res) {
            ...
            if (res.length && res[0].status === 0) {
              ...
              this.sltBegin = res
              console.log(this.sltBegin, 'sltBegin')

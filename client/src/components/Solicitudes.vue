              ...
              <q-card :class="`${rol !== 3 && index === 1 ? 'q-mb-md' : ''} full-width cursor-pointer`" @click="verSlt(item2, index2)" :style="rol === 3 && index === 1 ? 'height: 370px;' : ''">
                <div class="row q-px-lg items-center justify-between no-wrap">
                  ...
                  <div class="row q-gutter-sm no-wrap">
                    <q-btn dense no-caps v-if="rol === 3 && ((item2.status > 1 && item2.status < 4) || item2.status === 6 || item2.status === 8)" size="12px" style="height: 25px; border-bottom-left-radius: 8px; border-bottom-right-radius: 8px; border-top-left-radius: 0px; border-top-right-radius: 0px;" color="primary" :label="$t('Reasignar')" @click="reasign = true"/>
